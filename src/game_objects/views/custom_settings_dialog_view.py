...
class CustomSettingsDialogView(Drawable):
    def __init__(self, custom_settings_dialog):
        ...
        self._num_processes_label_text = FONT_SECONDARY_MEDIUM.render('初始进程数量', True, Color.WHITE)
        self._num_ram_rows_label_text = FONT_SECONDARY_MEDIUM.render('内存排数', True, Color.WHITE)
        self._new_process_probability_label_text = FONT_SECONDARY_MEDIUM.render('新进程创建概率', True, Color.WHITE)
        self._io_probability_label_text = FONT_SECONDARY_MEDIUM.render('I/O 事件概率', True, Color.WHITE)
